...
public class ThreadWrite extends Thread{
	...
	public void run(){
		...
		while(canRun){
			...
			if(!lowPriorityTxQueue.isEmpty()){
			    ThreadWrite.yield(); //cerca di eseguire alla massima velocit� possibile (100% cpu) ma lasciando spazio agli altri thread
				this.scrivi();
			}
			else{
			    try {
	...
	private void scrivi(){
		...
		//Sleep per 100 ms
		try {
			sleep(200); // C'� un problema nei tempi, se non metto uno sleep cos� elevato invia e riceve 3 volte
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
